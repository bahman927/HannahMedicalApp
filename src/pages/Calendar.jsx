...
const Calendar = (props) => {
  ...
  //  const doctors = ['Dr. SmdocNameith', 'Dr. Johnson', 'Dr. Brown', 'Dr. Lee', 'Dr. Patel'];
...
useEffect(() => {
  ...
  console.log('storedData in 2nd useEffect = ', storedData)
  if (storedData) {
...
useEffect(() => {
  ...
  if (bookedDays)
     ...
     const storedData = localStorage.getItem(`bookedDays_${selectedDoctor}`);
    //  JSON.parse(storedBookedDays);
     console.log('storedData in 3nd useEffect = ', storedData)
...
const handleTimeSlotBooking = (timeSlot) => {
        ...
        if (!patientInfo.name || !patientInfo.familyName){
          ...
          if (selectedDay && selectedTimeSlot && patientInfo.name && patientInfo.familyName) {

              updatedDays[selectedDoctor][currentYear][currentMonth][selectedDay].push({
                timeSlot: selectedTimeSlot,
                patient: { name: patientInfo.name,  familyName: patientInfo.familyName },
              })

             alert(`Booked ${timeSlot} on ${selectedDay} for ${patientInfo.name} ${patientInfo.familyName}`);
          ...
          //  console.log('updatedDays',  updatedDays)
           alert('Please enter patient name and family name.');
          }
        })
      }
// -----------------------------------------------------
...
const isDayBooked = (day) => {
  ...
  //   return `${booking.patient.name} ${booking.patient.familyName}`;
  // };
...
return (
    ...
    <div className="flex flex-col items-center">
        ...
        <label className="mb-4">
          Select Doctor:
          <select value={selectedDoctor} onChange={handleDoctorChange} className="ml-2 p-2 border rounded">
        ...
        {/* Month Navigation */}
        <h1 className='text-2xl mt-16 mb-2 text-blue-800 font-semibold'> Appointment Time Slot for {selectedDoctor}</h1>
        <div className="flex items-center justify-between w-full max-w-md mb-5">
         ...
         {/* Days Grid */}
        <div className="grid grid-cols-7 gap-2 w-full max-w-md mb-4">
            {daysArray.map((day) => (
              <button
                key={day}
        ...
        {selectedDay && (
          ...
          {selectedTimeSlot && submitted && (
                  ...
                  <div className="patient-info text-xl ml-5 mt-4 mb-4"> 
                    ...
                    <input className='mr-4'
                      ...
                      value={patientInfo.familyName}
                       onChange={handlePatientInfoChange}
                       ref={patientInputRef2}
                  
                    />
                     
                    <button className='mb-5' onClick={() => handleTimeSlotBooking(selectedTimeSlot)}>Confirm Booking</button>
                  </div>
